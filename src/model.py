...
def res_conv(X, filters, base, s):
    ...
    X = Conv2D(filters=F2, kernel_size=(2,2), strides=(s,s), padding='same', name=name_base + '1_conv_2', kernel_initializer=glorot_uniform(seed=0))(X)
...
def res_identity(X, filters, base):
    ...
    ##### Branch1 #####
    # First component of Branch1 
    X = BatchNormalization(axis=-1, name=name_base + '_1_bn_1')(X)
    Shortcut= Activation('relu', name=name_base + '_1_relu_1')(X)
    X = Conv2D(filters=F1, kernel_size=(1,1), strides=(1,1), padding='valid', name=name_base + '_1_conv_1', kernel_initializer=glorot_uniform(seed=0))(Shortcut)

    # Second component BranchOut 1
    X1 = BatchNormalization(axis=-1, name=name_base + '_1_ConvBn_2')(X)
    X1 = Activation('relu', name=name_base + '_1_ConvRelu_2')(X1)
    X1 = Conv2D(filters=F2, kernel_size=(2,2), dilation_rate=(2, 2),strides=(1,1), padding='same', name=name_base + '_1_Conv_2', kernel_initializer=glorot_uniform(seed=0))(X1)
    
    # Second component BranchOut 2
    X2 = BatchNormalization(axis=-1, name=name_base + '_1_SepBn_2')(X)
    X2 = Activation('relu', name=name_base + '_1_SepRelu_2')(X2)
    X2 = SeparableConv2D(filters=F2, kernel_size=(2,2), dilation_rate=(2, 2),strides=(1,1), padding='same', name=name_base + '_1_SepConv_2', kernel_initializer=glorot_uniform(seed=0))(X2)
    
    # Second component Add-BranchOut
    X = Add(name=base + '_Add-2branches')([X1, X2])
    
    # Third component of Branch1
    X = BatchNormalization(axis=-1, name=name_base + '_1_bn_3')(X)
    X = Activation('relu', name=name_base + '_1_relu_3')(X)
    X = Conv2D(filters=F3, kernel_size=(1,1), strides=(1,1), padding='valid', name=name_base + '_1_conv_3', kernel_initializer=glorot_uniform(seed=0))(X)    
    
    # Final step: Add Branch1 and the original Input itself
    X = Add(name=base + '_Add')([X_shortcut,X])
...
def res_identity(X, filters, base):
    ...
    X2 = BatchNormalization(axis=-1, name=name_base + '_1_SepBn_2')(X)
    X2 = Activation('relu', name=name_base + '_1_SepRelu_2')(X2)
    X2 = SeparableConv2D(filters=F2, kernel_size=(2,2), dilation_rate=(2, 2),strides=(1,1), padding='same', name=name_base + '_1_SepConv_2')(X2)
...
def EncoderDecoder(X, name_base):
    X = MaxPooling2D((3,3), strides=(2,2), padding='same', name = name_base + '_Downsample1')(X)
    #X = Conv2D(outgoing_depth, (2,2), strides=(1,1), dilation_rate=(2,2), padding='same', name = name_base + '/DC1', kernel_initializer=glorot_uniform(seed=0))(X)    
    X = UpSampling2D(size=(2, 2),interpolation='bilinear',name = name_base + '_Upsample1')(X)
    X = Activation('sigmoid', name = name_base + '_Activate')(X)
    return X
...
def RDBI(X, filters, base, number):
    for i in range(number):
        X = res_identity(X, filters, base+ '_id_'+str(1+i))
    return X
