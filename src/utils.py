...
def callback_for_training(tf_log_dir_name='./log/',patience_lr=10,snapshot_name=None):
    ...
    if snapshot_name == None:
        checkpointer = callbacks.ModelCheckpoint(filepath="optic-net.{epoch:02d}-{val_acc:.2f}.keras",
                                verbose=0,
                                monitor='val_acc')
    else :
        checkpointer = callbacks.ModelCheckpoint(filepath=snapshot_name+".{epoch:02d}-{val_acc:.2f}.keras",
                                verbose=0,
                                monitor='val_acc')
    cb[1] = checkpointer
